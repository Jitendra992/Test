...
const Section10 = () => {
  ...
  const computerPeripherals = data?.data?.data?.find(
    (i) => i.category === "Computer & Peripherals"
  );
  const products = computerPeripherals?.product_deatils || [];
    ...
    <div className="h-full w-full px-4 py-8">
      <div className="flex flex-col lg:flex-row justify-between">
        ...
        <div className="h-[10vh] w-full flex justify-between p-4 items-center bg-[#F3F4F6]">
            <p className="text-base sm:text-lg md:text-xl font-semibold">Computer Peripherals</p>
            <p  onClick={()=>navigate("/product",{state:{c_id:computerPeripherals?.id}})} className="text-base sm:text-lg md:text-xl">See All</p>
          </div>
            ...
            {products.map((item, index) => (
              <div key={index} className="px-2">
                ...
                <div onClick={() => navigate(`/details/${item?.id}/${item?.variant_id}/${item?.slug}`)}
                 className="bg-white rounded-2xl shadow-md p-4 mx-auto max-w-xs hover:scale-105 transition-transform duration-300 border border-gray-200 h-full">
                  ...
                  <p className="text-sm mb-1 text-green-500 font-semibold">
                    {item?.stock_sataus}
                  </p>
