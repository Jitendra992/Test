...
const Section3 = () => {
  ...
  const Mobile = data?.data?.data?.filter(
    (i) => i.category === "Mobiles & Tablets"
  );
    ...
    <div className="h-full w-full bg-gradient-to-b from-[#f0f4f8] to-[#e0f7fa] py-10 px-4 ">
      ...
      <div className="w-full lg:w-[30%] hidden lg:block">
        ...
        <div className="h-[10vh] w-full flex justify-between p-4 items-center bg-[#F3F4F6]">
            <p className="text-base sm:text-lg md:text-xl font-semibold">Mobiles & Tablet</p>
            <p  onClick={()=>navigate("/product",{state:Mobile?.[0]?.id})} className="text-base sm:text-lg md:text-xl">See All</p>
          </div>
          {isLoading ? (
            <>
            ...
            <Slider {...settings}>
              {Mobile?.[0]?.product_deatils?.map((item, index) => (
